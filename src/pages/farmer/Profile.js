      ...
      <div className="max-w-2xl mx-auto p-4 lg:p-6">
        ...
        <div className="bg-white rounded-xl shadow p-6 mb-6 text-center">
          <div className="relative w-24 h-24 mx-auto mb-4">
            {profilePhoto ? (
              <img src={profilePhoto} alt="Profile" className="w-24 h-24 rounded-full object-cover border-4 border-green-100" />
            ) : (
              <div className="w-24 h-24 bg-green-100 rounded-full flex items-center justify-center">
                <User className="h-12 w-12 text-green-600" />
              </div>
            )}
            <label htmlFor="profile-photo-input" className="absolute bottom-0 right-0 bg-green-600 text-white p-2 rounded-full cursor-pointer hover:bg-green-700 shadow">
              <Camera className="h-4 w-4" />
            </label>
            <input type="file" id="profile-photo-input" accept="image/*" className="hidden" onChange={handleProfilePhotoChange} />
          </div>
